...
export async function getUserFromAuth(userId) {
  try {
    const response = await axios.get(`http://127.0.0.1:8001/auth/users/id/${userId}`);
...
export async function getUserByUsername(username) {
  try {
    const response = await axios.get(`http://127.0.0.1:8001/auth/users/nickname/${username}`);
    return response.data;
  } catch (error) {
...
export async function getUsersFromAuth(userIds) {
  ...
  try {
    ...
    const response = await axios.get(`http://127.0.0.1:8001/auth/users`);
