...
import { Message, ChatRoom } from "./src/models/types.js";
import {
  WebScoketAdapter,
  registerWebSocketAdapter,
} from "./src/WebSocketAdapter.js";
...
const start = async () => {
  try {
    ...
    setupWebSocketHandlers(wsAdapter);
...
function setupWebSocketHandlers(wsAdapter) {
  ...
  wsAdapter.on("user:connect", async ({ clientId, payload }) => {
    ...
    try {
      ...
      //     const friendClientId = onlineUsers.get(friendUsername);
      //     if (friendClientId) {
      //       wsAdapter.sendTo(friendClientId, 'user:online', { username });
      //     }
      //   });
      // }
  ...
  wsAdapter.on("user:disconnect", async ({ clientId }) => {
    ...
    if (disconnectedUser) {
      ...
      // try {
      //   const user = await getUser(disconnectedUser);
      //   if (user && user.friends) {
      //     user.friends.forEach(friendUsername => {
      //       const friendClientId = onlineUsers.get(friendUsername);
      //       if (friendClientId) {
      //         wsAdapter.sendTo(friendClientId, 'user:offline', { username: disconnectedUser });
      //       }
      //     });
      //   }
      // } catch (err) {
      //   fastify.log.error('Error getting user data during disconnect', err);
      // }
    }
  });
  ...
  wsAdapter.on("user:block", async ({ clientId, payload }) => {
    ...
    wsAdapter.on('user:unblock', async ({ clientId, payload }) => {
      try {
        ...
        wsAdapter.sendTo(clientId, 'error', { message: 'Failed to unblock user' });
      }
    });
}
