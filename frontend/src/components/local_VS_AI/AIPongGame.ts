...
const difficultySpeeds = {
  ...
  const update = () => {
    ...
    if (state.ballY <= 0 || state.ballY >= canvas.height) state.ballSpeedY *= -1;

    // Ball collision detection with paddles
    const hitLeftPaddle = state.ballX - state.ballSize <= state.paddleWidth &&
      state.ballY >= state.playerY &&
      state.ballY <= state.playerY + state.paddleHeight;

    const hitRightPaddle = state.ballX + state.ballSize >= canvas.width - state.paddleWidth &&
