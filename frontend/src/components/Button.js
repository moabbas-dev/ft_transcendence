...
export const Button = createComponent((props) => {
	...
	button.className = `flex items-center justify-center hover:opacity-80 hover:cursor-pointer transition-all duration-300 px-4 py-2 bg-[var(--main-color)] rounded-full ${props.styles}`;
	button.addEventListener(props.eventType, props.onClick);
	useCleanup(props.onClick)
	return button;
})