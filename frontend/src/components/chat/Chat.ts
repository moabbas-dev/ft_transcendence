...
export const Chat = createComponent((props: { activeUser?: { nickname: string, id: number, full_name: string } }) => {
                ...
                <header class="flex rounded-full mt-2 h-fit items-center justify-between py-3 px-2 bg-white">
                    <div class="flex">
                        ...
                        <div class="flex items-center justify-center gap-1 sm:gap-2">
                            ...
                            <div>
                                <p class="friend_name text-base sm:text-xl">${activeUser ? `${activeUser.full_name} - ${activeUser.nickname}` : 'Select a chat'}</p>
                            </div>
                        </div>
                    </div>
                    ${activeUser ? `
    ...
    const renderMessages = () => {
        ...
        return messages.map(message => {
            ...
            return `
                <div class="message-wrapper ${isCurrentUser ? 'self-end' : 'self-start'} max-w-[75%] my-1">
                    <div class="message ${isCurrentUser ? 'bg-pongblue text-white' : 'bg-white text-black'} rounded-2xl px-3 py-2">
                        <div class="message-content break-words">${message.content}</div>
                        <div class="message-time text-xs opacity-70 text-right">${messageTime}</div>
                    </div>
                </div>
            `;
        }).join('');
    ...
    const initWebSocketEvents = () => {
        ...
        chatService.on('user:typing', (data) => {
            ...
            if (activeUser && username === activeUser.nickname) {
                ...
                container.querySelector('#message-container')?.appendChild(typingIndicator);
