...
export const TwoFactorSend = createComponent((props: TwoFactorSendProps) => {
	...
	const verifyButton = Button({
		...
		onClick: async (e: Event) => {
			...
			try {
				const userData = await axios.post(`http://localhost:8001/auth/twoFactor/login/${store.sessionId}`, body, {headers: {"x-api-key": import.meta.env.VITE_AUTHENTICATION_API_KEY}});
