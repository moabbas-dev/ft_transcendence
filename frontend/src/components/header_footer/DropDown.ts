...
export const DropDown = createComponent(() => {
	...
	container.className = `account-list ${store.userId? 'py-1' : 'py-2'} rounded-md shadow-md shadow-white ${localStorage.getItem('selectedLanguage') === 'ar'? 'left-0' : 'right-0'} text-nowrap absolute z-[999] top-[48px] sm:top-[54px] bg-white text-pongdark hidden flex-col animate-fade-down animate-once animate-duration-300`
	container.innerHTML = `
		${!store.userId? `
