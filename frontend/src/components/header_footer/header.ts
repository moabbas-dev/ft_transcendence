...
export const Header = createComponent(() => {
    ...
    async function fetchUserNotifications(userId: number): Promise<NotificationData[] | null> {
        try {
            const response = await axios.get(`http://localhost:3002/api/notifications/user/${userId}`, {headers: {"x-api-key": import.meta.env.VITE_NOTIFICATION_API_KEY}});
    ...
    async function markAllNotificationsAsRead() {
        try {
            await axios.patch(`http://localhost:3002/api/notifications/read/all/${userId}`, undefined, {headers: {"x-api-key": import.meta.env.VITE_NOTIFICATION_API_KEY}});
