...
export default {
	render: (container: HTMLElement) => {
		...
		container.innerHTML = `
			...
			<div id="content" class="flex max-sm:flex-col max-sm:items-center max-sm:justify-around max-sm:py-4 flex-1 container mx-auto px-2 w-full text-white overflow-x-hidden sm:overflow-hidden">
				<div class="flex flex-col items-center justify-center gap-5 sm:gap-10 w-full sm:w-1/2 bg-pongdark">
					<h1 class="text-5xl font-semibold w-full text-center">${t('play.title')}</h1>
					<div class="flex flex-col gap-4">
						<button id="join-tournament" class="py-4 px-10 sm:px-20 text-xl bg-gradient-to-br from-pongblue to-[rgba(100,100,255,0.8)] hover:from-[rgba(100,100,255,0.9)] hover:to-pongblue hover:animate-pulse rounded-full">${t('play.tournaments.joinBtn')}</button>
						<button id="create-tournament" class="py-4 px-10 sm:px-20 text-xl bg-gradient-to-br from-pongblue to-[rgba(100,100,255,0.8)] hover:from-[rgba(100,100,255,0.9)] hover:to-pongblue hover:animate-pulse rounded-full">${t('play.tournaments.createBtn')}</button>
					</div>
				</div>
				<div id="game-mode-details" class="flex flex-col items-center justify-around max-sm:w-full w-1/2 bg-pongdark">
						<div class="relative w-full h-[4rem] sm:h-[8rem] flex items-center justify-center">
							...
							<span id="text-join" class="text-[2rem] sm:text-[3rem] md:text-[5rem] text-center font-bold absolute transition-opacity duration-500 opacity-0">${t('play.tournaments.join')}</span>
						</div>
						...
						<div class="relative w-full h-[4rem] sm:h-[8rem] flex items-center justify-center">
							...
							<span id="text-create" class="text-[2rem] sm:text-[3rem] md:text-[5rem] text-center font-bold absolute transition-opacity duration-500 opacity-0 ">${t('play.tournaments.create')}</span>
						</div>
				</div>
			</div>
		`;
		...
		onlineShowdownBtn?.addEventListener("click", () => {
			...
			const gameModeDetails = document.getElementById("game-mode-details");
			if (gameModeDetails) {
				heading.textContent = t('play.onlineGame.findingOpponent');
